...
const DIAPage = ({mqttData, initialData}) => {

  useEffect(() => {
    if (initialData && Object.keys(initialData).length > 0) {
      const payload = JSON.stringify(initialData); // Convert the initialData to a JSON string
      console.log("initialData is here: ", payload);

      // Publish the initialData to the 'device/sts' topic
      const client = createMqttConnection();
      client.publish(
        "emit/content/bleData/dia",
        payload,
        { qos: 1 },
        (err) => {
          if (err) {
            console.error("Failed to publish DIA initialData to MQTT:", err);
          } else {
            console.log("DIA initialData successfully published to MQTT:", payload);
          }
        }
      );
    }
...
export async function getServerSideProps(context) {
  ...
  if (client) {
    ...
    client.subscribe("emit/content/bleData/dia", (err) => {
      if (err) {
